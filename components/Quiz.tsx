...
export const Quiz: React.FC<QuizProps> = ({ quizData }) => {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 my-4 border border-indigo-500 shadow-lg">
      <h2 className="text-2xl font-bold text-indigo-300 mb-4 border-b border-gray-600 pb-2">Cosmic Quiz Time!</h2>
      <div className="space-y-6">
        {quizData.questions.map((q, index) => (
          <div key={index} className="bg-gray-700 p-4 rounded-md">
            <p className="font-semibold text-lg text-white mb-3">{index + 1}. {q.question}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {q.options.map((option, optIndex) => (
                <button
                  ...
                  disabled={submitted}
                  className={`w-full text-left p-3 rounded-md text-white transition-all ${getButtonClass(index, option)}`}
                >
                  {option}
                </button>
              ))}
      ...
      {!submitted ? (
         <button 
           ...
           className="mt-6 w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg transition-colors"
         >
           Submit Answers
         </button>
      ) : (
        <div className="mt-6 text-center bg-gray-900 p-4 rounded-lg">
            ...
            <p className="text-2xl text-indigo-300 mt-2">You scored {calculateScore()} out of {quizData.questions.length}</p>
        </div>
      )}
    </div>
  );
